...
public class HomeController {
	...
	@GetMapping("/register")
	// @GetMapping 주소창에 register을 입력 받으면 실행됨
	...
	// @PostMapping 입력폼에서 submit버튼을 눌렀을 때 실행됨
	 ...
	 * 							  defualtValue의 값은 문자열로 설정되지만 매개변수로 대입될 때는 해당타입으로 형변환된다.
	 ...
	 * 		@PathVariable			요청 URL 경로에 포함된 파라미터값과 매개변수를 메핑시키는 어노테이션
	 * 		@ModelAttribute			요청파라미터와 해당값을 저장하는 객체를 매핑시키는 어노테이션
	 * 		@RequestBody			요청메세지의 바디부 정보와 매개변수를 매핑시키는 어노테이션(Ajax)
	 * 		@Valid					요청파라미터값의 유효성 여부를 검증시키는 어노테이션
	 * 		Model					뷰에 전달할 정보를 저장하는 객체
	 * 		Errors					요청파라미터값의 유효성 결과를 저장하는 객체
	 * 		BindingResult			요청파라미터값의 유효성 결과를 저장하는 객체
	 * 		SessionStatus			세션에 저장된 정보를 삭제하는 객체
	 * 		기본자료형					요청파라미터값을 전달받는다.
	 * 		String					요청파라미터값을 전달받는다.
	 * 		사용자정의 객체				요청파라미터값을 전달받는다.
	 */
	// 값이 얼마 안될 때는 받는 jsp의 name에 지정된 변수명을 그대로 적으면 그 값을 전달 받을 수 있다.
	// 단 전달 받는 값이 많은 경우는 public String register(UserRegisterForm userRegisterForm)와 같이 객체를 만들어서 객체에 담은 후 전달받는다.
	@PostMapping("/login")
	public String login(String id, String password) {
		// 아이디, 비밀번호로 로그인 검증
		User user =  userService.login(id, password);
		
		// HttpSession 객체에 로그인 절차가 왼료된 사용자 정보를 저장한다.
	 ...
	 * 		@GetMapping		: 요청방식이 GET 방식이고 요청URL이 일치하는 요청핸들러 메소드와 매핑시킨다.
	 * 		@PostMapping	: 요청방식이 POST 방식이고 요청URL이 일치하는 요청핸들러 메소드와 매핑시킨다.
	 ...
	 * 		PUT		: 서버에 정보를 변경하는 요청
	 * 		DELETE	: 서버에 정보를 삭제하는 요청
	 *  * 일반적인 웹 어플리케이션에서는 GET, POST 두가지 방식을 사용한다.
	 *  * Rest 방식의 웹 어플리케이션에서는 GET, POST, PUT, DELETE 방식을 전부 사용한다.
	 */
	@RequestMapping("/logout")
	public String logout() {
		// HttpSession 객체에서 로그인된 사용자 정보를 삭제한다.
