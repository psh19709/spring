...
/**
 * 요청핸들러 메소드의 매개변수에 @LoginUser 어노테이션이 있는지 조사한다. <br/>
 * @LoginUser 어노테이션이 있으면 로그인이 필요한 작업을 요청한 것으로 판단하고, 
...
public class LoginCheckHandlerInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 요청핸들러 메소드 혹은 요청핸들러 메소드의 매개변수가 @LoginUser을 가지고 있는지 확인한다.
		boolean isLoginRequired = hasMethodAnnotation(handler) || hasParameterAnnotation(handler);
		
		// isLoginRequired가 false면 로그인여부와 상관없이 true를 반환한다 - 요청핸들러 메소드가 실행굄
		if (!isLoginRequired) {
			return true;
		}
