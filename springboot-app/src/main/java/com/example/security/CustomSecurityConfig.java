...
public class CustomSecurityConfig extends WebSecurityConfigurerAdapter {
	 ...
	 * 			csrf() 메소드는 csrf토큰 사용여부를 설정하는 CsrfConfigurer를 반환한다.
	 * 			authorizeHttpRequests() 메소드는 인가정책을 설정하는 AuthorizeHttpRequestsConfigurer를 반환한다.
	 */
	@Override
	protected void configure(HttpSecurity http) throws Exception {
		http
			...
			// 1. 인가정책 설정 종료
		.and()	// HttpSecurity 객체를 반환한다. 새 보안 설정에 필요한 객체(xxxConfigurer)를 반환하는 메소드를 호출해서 보안설정을 추가한다.
			// 2. 인증 정책 설정 시작
